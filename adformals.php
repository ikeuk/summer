            ...
            if(in_array($videoFileType,$extensions_arr)){
                ...
                if(($_FILES['file']['size'] > $maxsize) || ($_FILES["file"]["size"] == 0)) {
                    ...
                    if(move_uploaded_file($_FILES['file']['tmp_name'],$target_file)){
                        // Insert record
    $sql = "INSERT INTO inner_hosp(file,location,productname,price,description) VALUES('".$name."','".$target_file."','".$productname."','".$price."','".$text."')";

                        $stmt = $pdo->prepare($sql);
                        $res = $stmt->execute();
                       if($res){
                        $user_id = $pdo->lastInsertID();
                        header("location: adcasuals.php?result=1");
			...
			<div class="address-row">
				<div class="col-md-6 address-left wow fadeInLeft animated" data-wow-delay=".5s">
					<div class="address-grid">
						<h4 class="wow fadeIndown animated" data-wow-delay=".5s">ADD FORMALS </h4>
						 <form method="post" action="adformals.php" enctype='multipart/form-data'>
